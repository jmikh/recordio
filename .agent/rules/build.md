...
- When deploying Supabase edge functions, ALWAYS use `--no-verify-jwt` flag (e.g. `supabase functions deploy <name> --no-verify-jwt`). All edge functions handle auth internally via the Authorization header.