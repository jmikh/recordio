 ...
 */
serve(withBoundary('render-job-create', async (req) => {
    ...
    if (rpcError) throw new Error('render_job_get_or_create failed', { cause: rpcError });
    if (!jobResult) throw new Error('render_job_get_or_create returned null');
    ...
    const uploadUrl = await getS3SignedUrl(s3, putCmd, { expiresIn: 3600 });
    ...
    fetch(`${RENDER_WORKER_URL}/render`, {
        ...
        // Fire-and-forget — log only. If dispatch actually fails the worker
        // never picks up the job; the hook timeout / retry handles that.
