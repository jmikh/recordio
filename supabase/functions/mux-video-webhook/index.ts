...
/**
 * Mux Video Webhook — receives Mux webhook events
 *
 * Handles:
...
serve(async (req: Request) => {
    ...
    try {
        ...
        if (eventType === 'video.asset.ready') {
            ...
            if (!playbackId) {
                console.error('[mux-video-webhook] asset.ready but no playback_id:', assetId);
            ...
            if (rpcError) {
                console.error('[mux-video-webhook] mux_video_complete failed:', rpcError);
                return errorResponse('DB function failed', 500);
            }

            if (!result?.found) {
                console.warn('[mux-video-webhook] No pending mux_video for asset:', assetId);
                return jsonResponse({ ok: true, message: 'No matching pending row' });
            }

            console.log(`[mux-video-webhook] asset.ready: ${assetId} -> mux_video ${result.mux_video_id} completed`);
        ...
        if (eventType === 'video.asset.errored') {
            ...
            if (muxVideo) {
                ...
                console.log(`[mux-video-webhook] asset.errored: ${assetId} -> mux_video ${muxVideo.id} failed`);
            }
        ...
        console.error('[mux-video-webhook] Unexpected error:', err);
