 ...
 *   2. Service role key (from mux-video-upload): skips Pro check, uses admin client
 *
...
serve(async (req: Request) => {
    ...
    try {
        ...
        if (isServiceRole) {
            // Service role path: called internally by mux-video-upload
            // Look up project owner, skip Pro check
        ...
        // From here, both paths converge — we have projectId and userId
        ...
        // Generate signed upload URL for output (path defined by render_job_start)
        ...
        console.error('[render-start-job] Unexpected error:', err);
