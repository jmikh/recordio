 ...
 */
serve(withBoundary('shared-video-get', async (req) => {
    ...
    if (pending) return jsonResponse({ ...baseResponse, status: 'pending' });
    ...
    if (failed) return jsonResponse({ ...baseResponse, status: 'failed' });
