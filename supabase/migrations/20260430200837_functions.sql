...
-- ============================================================
-- Source: mux_video_resolve.sql
-- ============================================================
-- mux_video_resolve(p_project_id)
--
-- Atomically resolves a Mux upload job:
...
--   4. Dedup: existing rendering/uploading mux_video for current cloud_version
--   5. Check if completed render exists
--      - No render: create mux_video with status='rendering', call render_job_resolve
--      - Render exists: create mux_video with status='uploading'
--   6. Cancel stale in-progress mux_videos for this project
--   7. Insert new row
--
-- user_id is derived from the project — no need to pass it in.
--
-- Called by: edge function mux-video-sync
...
CREATE OR REPLACE FUNCTION public.mux_video_resolve(
    p_project_id UUID
)
RETURNS TABLE(
    mux_video_id UUID,
...
BEGIN
    ...
    IF NOT v_shared_exists THEN
        RETURN QUERY SELECT NULL::UUID, 'not_shared'::TEXT, FALSE, FALSE, NULL::TEXT, NULL::UUID, NULL::INT;
        RETURN;
    END IF;
    ...
    IF v_existing_id IS NOT NULL THEN
        RETURN QUERY SELECT v_existing_id, 'completed'::TEXT, FALSE, FALSE, NULL::TEXT, NULL::UUID, v_cloud_version;
        RETURN;
    END IF;

    -- 4. Dedup: existing in-progress mux_video for current cloud_version
    SELECT mv.id, mv.status INTO v_existing_id, v_existing_status
    FROM public.mux_videos mv
    WHERE mv.project_id = p_project_id
      AND mv.cloud_version = v_cloud_version
      AND mv.status IN ('rendering', 'uploading');

    IF v_existing_id IS NOT NULL THEN
        RETURN QUERY SELECT v_existing_id, v_existing_status, FALSE, FALSE, NULL::TEXT, NULL::UUID, v_cloud_version;
        RETURN;
    END IF;
    ...
    WHERE mv2.project_id = p_project_id
      AND mv2.status IN ('rendering', 'uploading');

    -- 7. If no render, also resolve a render job
    IF v_render_path IS NULL THEN
        SELECT rjr.job_id, rjr.render_storage_path
        INTO v_render_job_id, v_render_path
        FROM public.render_job_resolve(p_project_id, v_user_id) rjr;

        INSERT INTO public.mux_videos (project_id, user_id, cloud_version, render_job_id, status)
        VALUES (p_project_id, v_user_id, v_cloud_version, v_render_job_id, 'rendering')
        RETURNING mux_videos.id INTO v_new_id;

        RETURN QUERY SELECT v_new_id, 'rendering'::TEXT, TRUE, TRUE, v_render_path, v_render_job_id, v_cloud_version;
        RETURN;
    END IF;

    -- 8. Render exists — insert as uploading
    INSERT INTO public.mux_videos (project_id, user_id, cloud_version, render_storage_path, status)
    VALUES (p_project_id, v_user_id, v_cloud_version, v_render_path, 'uploading')
    RETURNING mux_videos.id INTO v_new_id;

    RETURN QUERY SELECT v_new_id, 'uploading'::TEXT, TRUE, FALSE, v_render_path, NULL::UUID, v_cloud_version;
END;
$$;
...
-- and cascades to any mux_videos waiting on this render.
--
-- On completed: mux_videos with this render_job_id move from 'rendering' → 'uploading'
-- On failed/canceled: mux_videos with this render_job_id are marked 'failed'
--
...
-- ============================================================
-- Source: render_job_resolve.sql
-- ============================================================
-- render_job_resolve(p_project_id, p_user_id)
--
-- Atomically resolves a render job:
...
--
-- Called by: edge function render-sync
...
CREATE OR REPLACE FUNCTION public.render_job_resolve(
    p_project_id UUID,
    p_user_id UUID
)
