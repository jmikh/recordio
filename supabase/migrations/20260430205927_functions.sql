...
--
-- Atomically resolves a mux_video row for a project:
--   1. Read project cloud_version
--   2. Cache hit: completed mux_video for current cloud_version
--   3. Dedup: existing rendering/uploading mux_video for current cloud_version
--   4. Cancel stale in-progress mux_videos, insert new row as 'rendering'
--
-- Does NOT touch render_jobs — the caller (mux-video-sync) handles rendering.
--
-- Called by: edge function mux-video-sync
-- Tables:   projects, mux_videos
...
DECLARE
    ...
    v_new_id UUID;
BEGIN
    -- 1. Read project cloud_version and user_id
    ...
    IF v_existing_id IS NOT NULL THEN
        RETURN QUERY SELECT v_existing_id, 'completed'::TEXT, FALSE, v_cloud_version;
        RETURN;
    END IF;

    -- 3. Dedup: existing in-progress mux_video for current cloud_version
    ...
    IF v_existing_id IS NOT NULL THEN
        RETURN QUERY SELECT v_existing_id, v_existing_status, FALSE, v_cloud_version;
        RETURN;
    END IF;

    -- 4. Cancel stale in-progress mux_videos for this project
    UPDATE public.mux_videos mv2
    SET status = 'canceled', updated_at = NOW()
    WHERE mv2.project_id = p_project_id
      AND mv2.status IN ('rendering', 'uploading');

    -- 5. Insert new row as 'rendering' (render_job_id set later by mux-video-sync)
    INSERT INTO public.mux_videos (project_id, user_id, cloud_version, status)
    VALUES (p_project_id, v_user_id, v_cloud_version, 'rendering')
    RETURNING mux_videos.id INTO v_new_id;

    RETURN QUERY SELECT v_new_id, 'rendering'::TEXT, TRUE, v_cloud_version;
END;
$$;
