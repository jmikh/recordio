...
-- ============================================================
-- Source: mux_video_mark_old_deleted.sql
-- ============================================================
-- mux_video_mark_old_deleted()
--
-- For each project that has a completed mux_video, soft-deletes all older
-- completed/failed/canceled rows (sets is_deleted = true). Pending rows
-- are never touched.
--
-- Called by: edge function mux-video-purge (Step A before purge)
-- Tables:   mux_videos

DROP FUNCTION IF EXISTS public.mux_video_mark_old_deleted();

CREATE OR REPLACE FUNCTION public.mux_video_mark_old_deleted()
RETURNS VOID
LANGUAGE sql
SECURITY DEFINER
AS $$
    ...
    FROM latest l
    WHERE mv.project_id = l.project_id
      AND mv.cloud_version < l.max_version
      AND mv.status != 'pending'
      AND mv.is_deleted = FALSE;
$$;
