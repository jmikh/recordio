...
- Dashboard share entry — 2026-09-04. ShareModal + useProjectMetaStore moved to `webapp/src/share/` (used by editor AND dashboard); modal takes `projectName` as a prop (editor passes the live store name, dashboard the list item name); `toShareMeta(cloudProject)` helper extracted in cloudProjectService; `ProjectShareMeta` gained `cloudVersion` so the mux publish works when the editor isn't tracking a live version. Card menu gets an owner-only "Share" item (free tier → ProUpgradeModal; hydrates the meta store via project-get, syncs sharePolicy/workspaceAccess back onto the card on close).
