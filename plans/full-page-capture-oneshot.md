...
> Status: implemented 2026-09-15 (all steps, including the multi-strip and footer-pass steps).
> Deviations from the design below: the pure planner lives in `extension/src/shared/fullPagePlan.ts`
> (imported by both the content script and the background), not under `background/`; the footer
> pass runs whenever bottom-anchored fixed elements exist (they are hidden on every tile, including
> tile 0), so single-tile pages also place them via the footer op; `body` is accepted as an inner
> scroller; the toast hides with `display: none`. Manual verification matrix: pending.
...
- `../shared/fullPagePlan.ts` (shared with the content script) — **pure, no `chrome`/DOM, unit-tested**: `classifyFixedRect`,
