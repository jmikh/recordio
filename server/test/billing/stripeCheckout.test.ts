 ...
 *
 * Single per-seat plan since billing revamp Step 1: no plan/seats in the
 * request, quantity is always 1.
 */
...
describe('POST /stripe-checkout', () => {
    ...
    it('defaults to yearly, quantity 1 — full session params', async () => {
        const { app, deps } = testApp();
    ...
    it('monthly resolves the monthly price, quantity still 1', async () => {
        const { app, deps } = testApp();
        const res = await post(
            app,
            { ...validBody, interval: 'monthly' },
            await userToken(),
        );
        expect(res.statusCode).toBe(200);
    ...
    it('contributes workspace/interval to the canonical request event', async () => {
        ...
        await post(app, { ...validBody, interval: 'monthly' }, await userToken());
