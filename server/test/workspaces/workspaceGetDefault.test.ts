 ...
 * Since revamp Step 2 the route only RESOLVES (stored default → oldest
 * owned → heal the pointer): the signup trigger owns workspace creation,
 * so no-workspace is a 500 invariant violation, and owners have no
 * workspace_members row (role is synthesized). Dedicated auth users per
 * test — the chain mutates default_workspace_id.
 */
...
describe.runIf(hasTestDb())('POST /workspace-get-default (e2e, real Postgres)', () => {
    ...
    async function freshUser(opts: { keepBootstrapWorkspace?: boolean } = {}) {
        const user = await seedAuthUser(pool, opts);
        createdUsers.push(user.id);
        return user;
    }

    it('resolves the signup-trigger workspace for a brand-new user, role synthesized admin', async () => {
        const user = await freshUser({ keepBootstrapWorkspace: true });
    ...
    it('500 when the user owns no workspace — signup bootstrap invariant, no silent create', async () => {
        const user = await freshUser(); // helper drops the trigger workspace
        const res = await post(testApp(), await userToken({ sub: user.id, email: user.email }));
        expect(res.statusCode).toBe(500);
        expect(res.json()).toEqual({ error: 'No workspace for this account' });
    });
    ...
    it('returns the stored default when the membership is still live (with subscription seats)', async () => {
        ...
        await seedSubscription(pool, { workspaceId: stored.id, seats: 4 });
        await pool.query(
