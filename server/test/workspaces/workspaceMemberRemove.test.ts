/**
 * POST /workspace-member-remove — Part 2 Batch 3; seat pre-purchase
 * (plans/seat-prepurchase-oneshot.md): removing a creator/admin frees
 * their purchased seat but never touches Stripe — the seat count only
 * changes on the billing page.
...
describe.runIf(hasTestDb())('POST /workspace-member-remove (e2e, real Postgres)', () => {
    ...
    it('removing a creator never touches Stripe or email: the purchased seat count is unchanged', async () => {
        const ws = await workspaceWithAdmin();
        const member = await seedAuthUser(pool);
        createdUsers.push(member.id);
        await seedWorkspaceMember(pool, { workspaceId: ws.id, userId: member.id, role: 'creator' });
        await seedSubscription(pool, { workspaceId: ws.id, seats: 2 });
