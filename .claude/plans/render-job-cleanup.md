...
## Context
We refactored `render_job_start` to atomically handle cache-hit / dedup / cancel / insert. During that work we identified cleanup: the `quality` column is always `'1080p'` and unnecessary, and `output_storage_path` on `render_jobs` should be renamed to `render_storage_path` to match the projects table. `storage-download-url` should go back to accepting raw `storagePath` since it's also used by `CloudStorage.requestDownloadUrl` for media downloads.
...
### 4. Revert `storage-download-url/index.ts` back to `storagePath` mode
...
- On completion, call `storage-download-url` with `{ storagePath: job.render_storage_path }`
...
- `supabase/functions/storage-download-url/index.ts`
